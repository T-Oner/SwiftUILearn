...
//  HelloWorld
//
//  Created by 冯旭超 on 2020/3/28.
//  Copyright © 2020 冯旭超. All rights reserved.
//

import Foundation

enum CalculatorBarin {
    case left(String) //1
    case leftOp(
        left: String,
        op: CalculatorButtonItem.Op) //2
    case leftOpRight(
        left: String,
    ...
    var output: String {
        ...
        return format.string(from: value as NSNumber)!
    }
    
    func apply(item: CalculatorButtonItem) -> CalculatorBarin {
        switch item {
        case .digit(let num):
    ...
    private func applyDot() -> CalculatorBarin {
        switch self {
        case .left(let left):
    ...
    private func apply(op: CalculatorButtonItem.Op) -> CalculatorBarin {
        switch self {
        case .left(let left):
            switch op {
    ...
    private func apply(command: CalculatorButtonItem.Command) -> CalculatorBarin {
        switch command {
        case .clear:
            return .left("0")
        case .flip:
